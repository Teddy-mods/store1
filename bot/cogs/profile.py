...
def informacoes(update: Update, context: CallbackContext):
    keyboard = [
        ...
        InlineKeyboardButton(f'✂️ Deletar sua conta', callback_data='encerrar_conta')]]
    query = update.callback_query
    
    if asyncio.run(check_config('afiliado'))[1] == '1':
        keyboard.append([InlineKeyboardButton(f'👥 Afiliados', callback_data='afiliados')])
    
    keyboard.append([InlineKeyboardButton(f'⬅️ Voltar', callback_data='main')])

...
def is_live(content):
    ...
    if len(a) == 0:
        ...
        if len(b) == 0:
            return False, '⚠️ - #Reprovado - Request inválida não foi possível obter resposta. (Bad Request)'
        
        else:
            return True, '✅ - #Aprovado - Transação capturada com sucesso. (00 - Success)'

    else:
        return False, '❌ - #Reprovado - Transação recusada. (Autorização Negada)'
...
def curl_request(command, cc):
    ...
    try:
        ...
        return False, 'Não foi possível capturar esta request'

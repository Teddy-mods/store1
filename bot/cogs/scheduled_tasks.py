...
def scheduled_tasks(context):
    ...
    text2 = 'O telegram limitou essa ação, não será possível deletar.'
...
def backup_task(context):
    donos = adm_list()
    for dono in donos:
        context.bot.send_document(chat_id=dono, document=open('database.db','rb'), caption='| *Sistema de Backup (Diário)*\n\n_Este backup é feito a cada 12 horas, por segurança._', parse_mode='Markdown')


def payment_checker(context):
    print('\n\n----- FOI DETECTADO UMA NOVA RECARGA VIA PIX ------\n\n')
    path = 'temp'
    files = [f for f in os.listdir(path) if os.path.isfile(os.path.join(path, f))]
        ...
        if time_diference(str(criado_em)) == True:
            try:
                if tipo == 'mp':
                    ...
                    print('O pagamento se encontra:', status_order, 'Usuario do pagador:', user_nome)

                    if status_order == True:
                        if asyncio.run(pesquisar_recarga(id_p)) is None:
                            print('Estamos preparando o envio do saldo para:', user_nome)
                            asyncio.run(add_saldo(str(id_user), str(preco)))
                            print('Novo saldo adicionado com sucesso para:', user_nome)
                ...
                print(f'Ocorreu uma falha interna ao enviar o saldo : {e}')

...
def ferramentas(update: Update, context: CallbackContext):
    ...
    texto = '⚠️ As nossas ferramentas estão desativadas, por favor tente novamente mais tarde.'
...
def chk(update: Update, context: CallbackContext):
    ...
    if asyncio.run(check_config('checker_publico'))[1] == '1' or user_id in donos:
                ...
                if len(ccs) > 0:
                    for cc in ccs:
                        ...
                        if check[0]:
                            ...
                            text = '🚫 Função não habilitada para este bot.'

                        else:
                            #texto = '❌ | *Erro*\n\nO formato válido deve ser algo como: `/chk xxxxxxxxxxxxxxxx|xx|xx|xxx`'
                            texto = '🚫 Função não habilitada para este bot.'
        ...
        update.message.reply_text(text='❌ Este serviço não está habilitado, tente novamente mais tarde.', parse_mode='Markdown')

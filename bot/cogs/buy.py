...
def comprar(update: Update, context: CallbackContext):
    ...
    if check_status(user_id):
        ...
        keyboard = [
            [InlineKeyboardButton(f'💳 Unidades', callback_data='unitaria'),
            InlineKeyboardButton(f'🎲 Aleatórios', callback_data='aleatoria'),],
            [InlineKeyboardButton(f'🔃 Pacotes MIX', callback_data='mix'),
            InlineKeyboardButton(f'🔎 Procurar', callback_data='search')],
            [InlineKeyboardButton(f'◀︎ Voltar', callback_data='main')]]

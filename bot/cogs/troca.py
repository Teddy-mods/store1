...
def troca(update: Update, context: CallbackContext):
    ...
    if t[0] == True and int(t[1].split(':')[0]) <= tempo_de_troca:
        if asyncio.run(check_config('checker'))[1] == '1':
            a = query.bot.send_message(chat_id=query.message.chat.id, text=f"🆘 | *Estamos verificando a possibilidade de sua compra ser trocada.*", parse_mode='Markdown')
            recheck = checker(cc)
            if recheck[0] == False:
                context.bot.edit_message_text(chat_id=query.message.chat_id, message_id=a.message_id, text=f"✅ | _Sua troca foi aceita com sucesso._\n\n*A sua CC está sendo trocada, por favor aguarde.*", parse_mode='Markdown')
                while True:
                    contador_die = 0
                    contador_die += 1
                    repetidos = []
                            ...
                            if cc[0] == True and not cc[1] == []:
                                ...
                                if not credit_card in repetidos:
                                    ...
                                    if check[0] == True and not a_cc_foi_usada(credit_card):
                                        ...
                                        status = f'\n\n *⚙️ | Status do cartão:* `✅ (LIVE) -> Retorno da Gateway: {check[1]}`'

                                        query.bot.send_message(chat_id=query.message.chat.id, text=f"💜 | _Troca realizada com sucesso._\n\n*💳 | Cartão formatado*: `{numero}|{expiracao.replace('/', '|')}|{cvv}`\n*🇧🇷 | Bandeira*: `{bandeira}`\n*💵 | Nível*: `{categoria}`\n*🏦 | Banco*:  `{banco}`\n\n*📄 | Nome completo*: `{nome}`\n*📄 | CPF*: `{cpf}`"+status, parse_mode='Markdown')
        ...
        if int(t[1].split(':')[0]) <= tempo_de_troca:
            text = f'❌ Para evitar várias trocas para obter vantagens em nosso bot, você não pode solicitar a troca. Se passou: {t[1]} desde a contratação...'
        else:
            text = f'❌ | Infelizmente a sua troca não pôde ser feita, pois se passou {tempo_de_troca} minuto(s)'

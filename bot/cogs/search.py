...
def search(update: Update, context: CallbackContext):
    ...
    if check_status(user_id):
        ...
        keyboard.append([InlineKeyboardButton(f'⬅️ Voltar', callback_data='m1')])
        ...
        try:
            ...
            if query.lower().split()[0] == 'bin':
                if asyncio.run(check_config('pesquisar_bin'))[1] == '1':
                    ...
                    if len(results2) >= 1:
                        q = len(results2)
                        results.append(InlineQueryResultArticle(type='article', thumb_url='https://images.emojiterra.com/google/android-pie/512px/1f575.png', id=str(uuid4()),title=cards_found.format(q),description=see_cards,input_message_content=InputTextMessageContent(f"{q} {found_for_bin}: {query}"),))
                        for r in results2:
                            results.append(r)
                ...
                if asyncio.run(check_config('pesquisar_banco'))[1] == '1':
                    ...
                    if len(results2) >= 1:
                        q = len(results2)
                        results.append(InlineQueryResultArticle(type='article', thumb_url='https://images.emojiterra.com/google/android-pie/512px/1f575.png', id=str(uuid4()),title=cards_found.format(q),description=see_cards,input_message_content=InputTextMessageContent(f"{q} {found_for_bank}: {query}"),))
                        for r in results2:
                            results.append(r)
                ...
                if asyncio.run(check_config('pesquisar_bandeira'))[1] == '1':
                    ...
                    if len(results2) >= 1:
                        q = len(results2)
                        results.append(InlineQueryResultArticle(type='article', thumb_url='https://images.emojiterra.com/google/android-pie/512px/1f575.png', id=str(uuid4()),title=cards_found.format(q),description=see_cards,input_message_content=InputTextMessageContent(f"{q} {found_for_scheme}: {query}"),))
                        for r in results2:
                            results.append(r)
                ...
                if str(user_id) in donos:
                    ...
                    if len(results) >= 1:
                        q = len(results)
                        r2.append(InlineQueryResultArticle(type='article', thumb_url='https://images.emojiterra.com/google/android-pie/512px/1f575.png', id=str(uuid4()),title=f'{len(results)} usuários encontrados',description='Veja-os abaixo!',input_message_content=InputTextMessageContent(f"{len(results)} usuários encontrados para: {query}"),))
                        for r in results:
                            r2.append(r)
